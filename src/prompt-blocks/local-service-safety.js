...
1. **Normal local development stays easy.** For a page or app the user wants opened on this computer, use a dedicated project directory inside the ArkBrain-Agent sandbox and bind to 127.0.0.1 or ::1. "Open it in my browser" means local access; it does not imply LAN or public sharing.
...
5. **Keep the run observable.** Start long-running services with run_command action="start", mode="background"; retain the run_id, inspect startup output, and verify the actual URL before reporting success. A service started this way belongs to the current ArkBrain-Agent runtime; cancel it when the temporary preview is no longer useful. Do not turn a temporary preview into a startup daemon.`
