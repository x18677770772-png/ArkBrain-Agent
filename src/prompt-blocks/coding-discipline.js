...
1. **Skeleton first — run it immediately.** Write the smallest thing that can run (one entry file with stub content), start it, and verify it actually loads (run_command to run it, ArkBrain-Agent dedicated Chrome browser_navigate and inspect the automatic snapshot in its result). Only then add features. Never write the whole project across several files and run it for the first time at the end — by then every bug is buried under four files at once.
