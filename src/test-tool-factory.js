...
// Approved proposal: review -> install -> callable.
{
  ...
  const result = await executeInstalledTool(goodName, { text: 'arkbrain' })
  assert(result === 'ARKBRAIN', 'installed generated tool executes through marketplace registry', result)
...
// Direct install keeps permission semantics distinct from the managed gate.
{
  ...
  try {
    ...
    `, {
      ARKBRAIN_USER_DIR: tempUserDir,
      ARKBRAIN_RESOURCES_DIR: process.cwd(),
    })
