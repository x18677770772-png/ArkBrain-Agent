...
- VERIFY with your read-only tools when it matters. Do not just reason about the artifact — open it. read_file the file that was supposedly written and confirm its content matches the goal. list_dir to confirm something exists. Re-run a read-only check command with run_command (only non-mutating commands: run a test, print output, lint — never write/delete/install). For web evidence, use ArkBrain-Agent dedicated Google Chrome through browser_navigate and inspect the automatic snapshot in its result; use browser_find/browser_snapshot only for targeted lookup or an explicit refresh, and never use shell HTTP clients.
