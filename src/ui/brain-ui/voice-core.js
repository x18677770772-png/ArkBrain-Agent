...
export function createVoiceCore({ canvas, transcript, getChatInput, getSendMessage, getLang }) {
  ...
  // ─── 采集诊断（定位长语音丢字根因；localStorage 'arkbrain-voice-diag'='0' 关闭，默认开） ───
  const DIAG_ON = localStorage.getItem('arkbrain-voice-diag') !== '0';
  let diagTimer = null;
  ...
  // 也就修了「说到几十个字球变绿后不再出字」。localStorage 'arkbrain-voice-watchdog'='0' 可关。
  const WATCHDOG_ON = localStorage.getItem('arkbrain-voice-watchdog') !== '0';
