...
export function createAwakeningManager({
  ...
  function buildStartupSelfCheckDirections(checkState) {
    ...
    return [
      ...
      `3. Call speak text="正在检查方舟大脑专用 Chrome". Call ui_set({id:"self-check",kind:"selfcheck",intent:"inform",data:{phase:"running",step:3,total:3,name:"方舟大脑专用 Chrome",icon:"🌐"}}). Call browser_navigate once with url="https://example.com" and verify the page title/content from the automatic accessibility snapshot in that navigation result. Do not routinely call browser_snapshot; use it only if the navigation result lacks a snapshot. Call browser_close when done. Do not use any other networking tool. Record the actual result.`,
      `Continue even if a step fails. Then call ui_set({id:"self-check",kind:"selfcheck",intent:"inform",data:{phase:"done",results:[{name:"文件读写",status:"ok/error/skipped",note:"..."},{name:"热点面板",status:"ok/error/skipped",note:"..."},{name:"方舟大脑专用 Chrome",status:"ok/error/skipped",note:"..."}],overall:"ok/degraded/error"}}), replacing the placeholder values with the actual outcomes. Call complete_startup_self_check with the same evidence-based result map, then call ui_set with id="self-check" and remove=true.`,
    ].join('\n')
  }
