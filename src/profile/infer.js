...
const ROLE_RULES = [
  {
    ...
    keywords: [/arkbrain/i, /agent/i, /llm/i, /prompt/i, /context/i, /memory/i, /codex/i, /claude/i, /openai/i, /minimax/i],
...
function inferProjects(text) {
  const projects = []
  if (/arkbrain/i.test(text) || /方舟大脑/.test(text)) projects.push('ArkBrain-Agent')
